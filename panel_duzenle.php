...
if (isset($_GET['id'])) {
    ...
    $sorgu = $pdo->prepare("SELECT * FROM kullanici_giris WHERE id = ?");
...
if (isset($_POST['duzenle'])) {
    ...
    $guncelle_sorgu = $pdo->prepare("UPDATE kullanici_giris SET adsoyad = ?, mail = ?, sifre = ? WHERE id = ?");
    
    if (!empty($yeni_sifre)) {
        
        $guncelle_sonuc = $guncelle_sorgu->execute([$yeni_adsoyad, $yeni_mail, $yeni_sifre, $kullanici_id]);
    } else {
        $guncelle_sonuc = $guncelle_sorgu->execute([$yeni_adsoyad, $yeni_mail, $kullanici_id]);
    }

    if ($guncelle_sonuc) {
        echo '<div class="success-alert">
        <strong> KULLANICI GÜNCELLEME BAŞARILI </strong></div>';
...
<head>
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-alpha2/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-rbs5jQhjL6EfrG0tH9K1cE2tU5wAac3h/V3xUvLnE9Brx5yL94MPJ+axIF+Jmqs" crossorigin="anonymous">
    <style>
        body {
